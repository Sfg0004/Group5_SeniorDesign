    ...
    while(1):
        ...
        while(connected):
            if(lwMsg == "Get_Blockchain"):
                ...
                lw1PublicKey = comm.receivedatafromrequester(lw1)
                print("Public key received = " + lw1PublicKey)
                processedKey = rsa.PublicKey.load_pkcs1(lw1PublicKey.encode('utf-8'))
                comm.sendkeytoneighbor(lw1, rsa.encrypt((encryptionKey), processedKey))
                #Wraps up the connection
                lwMsg = " "
                lw1.close()
                connected = False
            else:
