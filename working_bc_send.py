        ...
        while(1):
            ...
            if(1):
                ...
                if child_pid == 0:            #   This code is executed by the child process\
                    ...
                    while(1):
                        time.sleep(4)
                        print(f"qsize child IMMEDIATLY: {parent_to_child.qsize()}")
                            ...
                            if len(validators) > 0:
                                ...
                                for block in candidateBlocks:
                                    if block.validatorName == lotteryWinner:
                                        ...
                                        #     file.write(message)
                                        blockchainMessage = assembleBlockchain()
                                        #parent_to_child.put(blockchainMessage)
...
def testprintBlockchain(): #makes the blockchain print better!
...
def assembleBlock(block): #assembles blockchain to be sent to requesting neighbor
    message = ""
    if block.transactionType == "Genesis":
        genesis = expandGenesisBlock()
        message = message + genesis
