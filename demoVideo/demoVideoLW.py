...
def main():
    ...
    #lwThread = threading.Thread(target=run_LW, args=(lw_to_full,))
        ...
        while(1):
            ...
            if(1):
                ...
                if child_pid == 0:            #   This code is executed by the child process\
                    ...
                    winnerThread = threading.Thread(target=pickWinner, args=(server_input_to_server,server_to_self_samaritan, parent_to_child,))
                    winnerThread.start()


                    if(not lw_to_full.empty()):
                        blockchain2 = lw_to_full.get()
                                            # accept incoming connections
...
def pickWinner(server_input_to_server,server_to_self_samaritan, parent_to_child):
    print("\nPicking winner...")
    while stopThreads == False:
        ...
        with validatorLock:   
            if len(validators) > 0:
                ...
                if(not lw_to_full.empty()):
                    candidateBlocks.append(lw_to_full.get())
